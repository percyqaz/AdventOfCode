...
let tick_player_hm(state: GameState): GameResult =
    let new_player_hp = state.PlayerHP - 1
    if new_player_hp <= 0 then
        Lose
    else
        { state with PlayerHP = new_player_hp } |> Next
...
let turn(state: GameState) : GameResult list =
    let ticked = tick state |> compose tick_player_hm
    seq {
